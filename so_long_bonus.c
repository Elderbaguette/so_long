...
/*   Updated: 2023/02/10 11:16:24 by aproust          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	playermove_wd(t_data *data, int i, int j, int event_trigger)
{
	...
	{
		if (data->map[j - 1][i] == 'E' || data->map[j - 1][i] == 'X')
			so_exit(data, data->map[j - 1][i]);
	...
	{
		if (data->map[j][i + 1] == 'E' || data->map[j][i + 1] == 'X')
			so_exit(data, data->map[j][i + 1]);
...
int	main(int ac, char **av)
{
	...
	if (fd == -1 || read_map(&data, fd) == 1)
		return (ft_printf("Error\nFailed attempt to read the map"), 1);
	...
	if (init_mlx(&data) == 1)
		return (ft_printf("Error\nFailed attempt to use Minilibx"), 1);
	if (alloc_img(&data) == 1 || so_long(&data) == 1 || counter(&data, 0) == 1)
